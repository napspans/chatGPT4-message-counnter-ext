...
# 機能追加
- "setInterval"が60秒で固定されているのを設定画面から1秒から10分で設定変更できるようにする。
  - 初期値 60秒
  - 10秒,20秒,30秒,40秒,50秒,1分～10分を選択項目にする。
- 同様に'MAX_TIMESTAMPS','TIME_TO_COUNT'も設定画面から可変にする。
  - MAX_TIMESTAMPS デフォルトは3時間
  - TIME_TO_COUNT デフォルトは50回
